...
namespace GK4
{
    class Cylinder : Figure
    {
        public Cylinder(int pieces)
        {
            ...
            Vector top_center = new Vector(0, 1, 0, 1);
            ...
            while (Math.Abs(curr - Pi / 2) > 0.0001)
            {
                ...
                Top.Add(new Vector((float)Math.Cos(curr), 1, (float)Math.Sin(curr), 1));
                curr += deg;
            }
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                Top.Add(new Vector(-Base[i][2], 1,Base[i][0], 1));//-y,x
            }
            for (int i = 0; i < length; i++)
            {
                Base.Add(new Vector(-Base[i][0],0, -Base[i][2],  1)); //-x,-y
                Top.Add(new Vector(-Base[i][0],1, -Base[i][2],  1));//-y,x
            }
            for (int i = 0; i < length; i++)
            {
                Base.Add(new Vector(Base[i][2],0, -Base[i][0],  1)); //y,-x
            ...
            for (int i = 0; i < Base_count - 1; i++)
            {
                Vector normal1 = new Vector(Top[i][0], 0, Top[i][2], 0);
                Vector normal2 = new Vector(Base[i + 1][0], 0, Base[i + 1][2], 0);
                Vector normal3 = new Vector(Base[i][0], 0, Base[i][2], 0);

                Vector normal4 = new Vector(Top[i][0], 0, Top[i][2], 0);
                Vector normal5 = new Vector(Top[i + 1][0], 0, Top[i + 1][2], 0);
                Vector normal6 = new Vector(Base[i + 1][0], 0, Base[i + 1][2], 0);
            ...
            Vector n1 = new Vector(Top[0][0], 0, Top[0][2], 0);
            Vector n2 = new Vector(Base[Base_count - 1][0], 0, Base[Base_count - 1][2], 0);
            Vector n3 = new Vector(Base[0][0], 0, Base[0][2], 0);

            Vector n4 = new Vector(Top[0][0], 0, Top[0][1], 0);
            Vector n5 = new Vector(Top[Base_count - 1][0], 0, Top[Base_count - 1][2], 0);
            Vector n6 = new Vector(Base[Base_count - 1][0], 0, Base[Base_count - 1][2], 0);
